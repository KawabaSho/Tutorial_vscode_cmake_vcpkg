...
### パッケージ管理
外部ライブラリをvcpkgで管理しているため、その情報の登録を `toolchainFile` で行っています。また、vcpkgはPC本体のパッケージを汚染しないために、pythonの仮想環境のような設定を行います。やり方はvcpkgでマニフェストを有効化させます。
...
`fmt` のパッケージを追加したら、`CMakeLists.txt` 内で以下のようにライブラリの追加を行います。
...
CMake tools を拡張機能でインストールしていると、VS Code のウィンドウの左下に `歯車 + build` ボタンがあると思われます。それを押し、`Default Config` を選択すれば、ビルドが行われます。 `歯車 + build` の右側にある再生ボタンを押せば、プロジェクトの実行結果を確認できます。出ない場合は `歯車 + build` をもう一度押すか、`F1`を押して、`CMake: Delete Cache and Reconfigure` を実行してください。それでも直らない場合は、`build` フォルダを丸ごと消して、`F1`を押して`Developer: Reload Window`を選択してください。それからまた `歯車 + build` を押すところから始めてください。
...
ビルド後、生成された実行ファイルを起動します。実行ファイルは `build\Debug\my_program.exe` にあります。VS Code のウィンドウの左下の `歯車 + build` の右側にある再生ボタンを押せば、プロジェクトの実行結果を確認できます。または、ターミナルで
...
MSVCのランタイム(CRT)は既定では、動的リンク`MD`/`MDd`であるので、他のライブラリが混ざっていても安全です。スタンドアロンの実行ファイルかつCRTの依存を減らしたい場合は、`CMakePresets.json` で `"cacheVariables"` の中身を以下のように変更してください。
...
依存ライブラリを `.dll` ファイルとして生成したい場合は、`CMakePresets.json` で以下のように変更してください。CRTは動的リンクしつつ、かつ、`build\Debug` 内で `fmt.dll` などが生成されます。
...
ちなみに、今回のプロジェクトでは、CRTは動的リンクしつつ依存関係は1つの実行ファイルの中に埋め込むために、`"x64-windows-static-md"` としています。また、CRTが静的で依存ライブラリが動的の場合は基本的に出来ないようです。理論上はできるようですが、
